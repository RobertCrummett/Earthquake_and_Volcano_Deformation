function [strike_slip,dip_slip,opening] = FiniteRectDisloc(s,x1,x2,d,dip,...
    W,L,mu,lambda)
...
if cos(dip) < 1e-15 && cos(dip) > -1e-15 % numeric zero
...
ss_u1 = @(zeta,eta) -s/(2*pi)*(zeta.*q./(R(zeta,eta).*(R(zeta,eta) + eta))...
    + atan2(zeta.*eta, q.*R(zeta,eta)) + I_1(zeta,eta)*sin(dip));
ss_u2 = @(zeta,eta) -s/(2*pi)*(y_tilde(eta).*q./(R(zeta,eta).*(R(zeta,eta) + eta))...
    + q*cos(dip)./(R(zeta,eta) + eta) + I_2(zeta,eta)*sin(dip));
ss_u3 = @(zeta,eta) -s/(2*pi)*(d_tilde(eta).*q./(R(zeta,eta).*(R(zeta,eta) + eta))...
    + q*sin(dip)./(R(zeta,eta) + eta) + I_4(zeta,eta)*sin(dip));

ds_u1 = @(zeta,eta) -s/(2*pi)*(q./R(zeta,eta) - I_3(zeta,eta)*sin(dip)*cos(dip));
ds_u2 = @(zeta,eta) -s/(2*pi)*(y_tilde(eta).*q./(R(zeta,eta).*...
    (R(zeta,eta) + zeta)) + cos(dip)*atan(zeta.*eta./(q.*R(zeta,eta))) -...
    I_1(zeta,eta)*sin(dip)*cos(dip));
ds_u3 = @(zeta,eta) -s/(2*pi)*(d_tilde(eta).*q./(R(zeta,eta).*...
    (R(zeta,eta) + zeta)) + sin(dip)*atan(zeta.*eta./(q.*R(zeta,eta))) -...
    I_5(zeta,eta)*sin(dip)*cos(dip));

op_u1 = @(zeta,eta) s/(2*pi)*(q.^2./(R(zeta,eta).*(R(zeta,eta) + eta))...
    - I_3(zeta,eta)*sin(dip)^2);
op_u2 = @(zeta,eta) s/(2*pi)*(-q.*d_tilde(eta)./(R(zeta,eta).*...
    (R(zeta,eta) + zeta)) - sin(dip)*(zeta.*q./(R(zeta,eta).*...
    (R(zeta,eta) + eta)) - atan(zeta.*eta./(q.*R(zeta,eta)))) - ...
    I_1(zeta,eta)*sin(dip)^2);
op_u3 = @(zeta,eta) s/(2*pi)*(q.*y_tilde(eta)./(R(zeta,eta).*...
